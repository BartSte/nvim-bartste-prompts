...
local function register_base_commands(commands)
...
local function register_prompt_command(command, type, prompt)
    vim.api.nvim_create_user_command(command, make_command(prompt, type),
        { range = true, nargs = "*" })
    log.debug("Registered %s command (prompt=%s)", command, prompt)
end

--- Registers user commands for different AI prompt types
--- Registers commands like `AiDocstrings`, `AiTypehints`, etc.
---@see vim.api.nvim_create_user_command
local function register_prompt_commands()
    register_prompt_command("AiDocstrings", "edit", "docstrings")
    register_prompt_command("AiTypehints", "edit", "typehints")
    register_prompt_command("AiRefactor", "edit", "refactor")
    register_prompt_command("AiExplain", "output", "explain")
    register_prompt_command("AiFix", "edit", "fix")
    register_prompt_command("AiTests", "edit", "unittests")
    register_prompt_command("AiAsk", "ask", "ask")
end
...
local function make_commands()
    ...
    register_base_commands(commands)
...
local function check_executables()
    ...
    if #missing > 0 then
        local message = string.format("Missing required executables: %s must be in PATH",
...
---@return boolean Result Returns true if required env vars are set, false otherwise
local function check_env_vars()
